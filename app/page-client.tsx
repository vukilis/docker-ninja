...
const SidebarItem = ({
	...
	return (
		...
		>
			...
			<div
				className={`flex items-center ${count !== undefined ? "justify-between" : "justify-start"} flex-1 pr-4 min-w-0 transition-all duration-300 ease-in-out ${collapsed ? "opacity-0 max-w-0 pointer-events-none" : "opacity-100 max-w-[200px]"}`}
			>
...
export default function Home({ initialView = "dashboard", initialAppSlug }: { initialView?: ViewMode; initialAppSlug?: string }) {
	...
	const handleAppSelect = (app: AppData) => {
		setSelectedApp(app);
		if (["About", "Community", "Sponsoring"].includes(currentView)) {
			setCurrentView("dashboard");
		}
		if (app.category) {
	...
	if (!isStarted) {
		return (
			<div className="min-h-screen bg-white dark:bg-[#0d1117] flex flex-col items-center justify-center p-6 transition-colors duration-700">
				{/* Syncs Landing State Metadata instantly */}
	...
	return (
		<div className="flex h-screen dark:bg-[#0d1117] text-slate-900 dark:text-slate-100 transition-colors overflow-hidden">
			{/* Monitors view variations, tab switches, and modal preview details */}
			<ClientMetadataController isStarted={isStarted} currentView={currentView} selectedApp={selectedApp} />
				...
				<div className="p-2.5 xl:p-4 h-full flex flex-col scrollbar-hide">
					...
					<nav className="flex-1 flex flex-col space-y-1 xl:space-y-2">
						...
						<div className="pt-3 mt-3 xl:pt-4 xl:mt-4 border-t border-slate-200 dark:border-slate-800">
							<div className={`grid gap-1.5 xl:gap-2 transition-all duration-300 ${sidebarCollapsed ? "grid-cols-1" : "grid-cols-2 xl:flex xl:flex-col"}`}>
								...
								<button
									...
									className={`relative group flex items-center h-9 xl:h-11 rounded-xl font-bold uppercase text-xs tracking-wider hover:bg-slate-200 dark:hover:bg-slate-800 transition-all duration-300 cursor-pointer justify-start overflow-hidden ${sidebarCollapsed ? "text-slate-500" : "text-slate-500 bg-slate-100/40 dark:bg-slate-900/20 xl:bg-transparent"} hover:text-emerald-600 dark:hover:text-emerald-400`}
								>
									...
									<div
										className={`flex items-center flex-1 pr-4 min-w-0 transition-all duration-300 ease-in-out relative z-10 ${sidebarCollapsed ? "opacity-0 max-w-0 pointer-events-none" : "opacity-100 max-w-[200px]"}`}
									>
								...
								<a
									...
									target="_blank"
									rel="noreferrer"
									className={`relative group flex items-center h-9 xl:h-11 rounded-xl font-bold uppercase text-xs tracking-wider hover:bg-slate-200 dark:hover:bg-slate-800 transition-all duration-300 justify-start overflow-hidden ${sidebarCollapsed ? "text-slate-500" : "text-slate-500 bg-slate-100/40 dark:bg-slate-900/20 xl:bg-transparent"} hover:text-purple-600 dark:hover:text-purple-400`}
								>
									...
									<div
										className={`flex items-center justify-between flex-1 pr-4 min-w-0 transition-all duration-300 ease-in-out relative z-10 ${sidebarCollapsed ? "opacity-0 max-w-0 pointer-events-none" : "opacity-100 max-w-[200px]"}`}
									>
										<span className="text-[10px] xl:text-xs truncate whitespace-nowrap overflow-hidden text-left">Report</span>
										<svg
											className="w-3 h-3 text-slate-400 opacity-60 transition-transform group-hover:translate-x-0.5 hidden xl:block shrink-0 ml-2"
											fill="none"
											viewBox="0 0 24 24"
											stroke="currentColor"
											strokeWidth="2.5"
