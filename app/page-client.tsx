...
export default function Home({ initialView = 'dashboard', initialAppSlug }: { initialView?: ViewMode; initialAppSlug?: string; }) {
	...
	const sortedCategories = useMemo(() => {
		return categories
		.filter(c => c !== "Dashboard" && c !== "categories")
	...
	const renderRecentlyViewed = () => {
		if (recentlyViewed.length === 0 || currentView === "categories") {
		return null;
	}
		...
		>
			...
			<nav className="flex-1 flex flex-col space-y-1 xl:space-y-2">
				...
				>
					...
					${currentView === 'dashboard' ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-800'}`}
					>
					{apps.length}
					</span>
				</div>
				...
				>
					...
					<span className={`px-2 py-0.5 rounded-md text-[10px] lg:text-[11px] font-sans shrink-0 ml-2
					${currentView === "categories" ? 'bg-white/20' : 'bg-slate-100 dark:bg-slate-800'}`}
					>
					{categories.length}
					</span>
				</div>
